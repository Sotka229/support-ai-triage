...
|---|---|
| PII-редакция (email, телефон, PAN+Luhn, CVV, паспорт, СНИЛС, ИНН, 20-значный счёт, адрес, IP) | Юридическое требование: пропуск PII во внешний LLM-API — инцидент, а не «падение метрики». Правило можно доказать построчно, модель — только статистически |
...
**Почему groundedness обязана стать NLI-проверкой, а не остаться пересечением токенов.** Это не
теоретическое соображение: PoC прогнан на живой локальной модели, и лексическая метрика пропустила
выдумку, противоречащую базе знаний, с оценкой 1.000 — при том, что ответ из другой темы она
отсекает. Дополнительно измерено, что разрыв растёт с числом фрагментов в контексте, потому что
сравнение идёт с их объединением. Отсюда требование к целевой реализации: проверять каждое
утверждение черновика против **каждого** фрагмента по отдельности, а не против конкатенации.
Замер, условия и практические следствия — в [SELF_REVIEW.md](../SELF_REVIEW.md).
